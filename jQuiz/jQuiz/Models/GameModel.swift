...
class GameModel {
  ...
  private struct URLs {
    ...
    static let cluesURL = "http://www.jservice.io/api/clues/"
...
//    static let cluesURLPath = "/api/clues/"
  ...
  public func fetchCategory(onCompletion: CompletionHandler? = nil) {
    ...
    Networking.fetchData(for: URL(string: articles)) { [weak self]
      ...
      switch result {
      case .success(let tutorialCollection):
        print(tutorialCollection.data)
  ...
  private func fetchClues(for category: Category) {
    ...
    let parameters = [
      URLs.cluesURLQueryItemCategory: categoryID,
      URLs.cluesURLQueryItemOffset: offset
    ]
    var cluesURLComponents = URLComponents(string: URLs.cluesURL)!
